...
class SchemaExtension extends Extension
{
        ...
        if ($curr) {
            ...
            if ($request) {
                ...
                if (! empty($getVars)) {
                    if (count($_GET) > 1) {
                        return;
                    } elseif (array_intersect(array_keys($getVars), Config::inst()->get(static::class, 'exempted_get_vars')) !== []) {
                        return;
                    }
                }
    ...
    {
        return 'https://validator.schema.org/#url=' . urlencode((string) $this->getOwner()->AbsoluteLink());
